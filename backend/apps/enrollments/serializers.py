...
class EnrollmentCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if student.tenant_id != tenant.id:
            raise AppException(
                "Student does not belong to your tenant", status.HTTP_400_BAD_REQUEST
            )

        if course.tenant_id != tenant.id:
            raise AppException(
                "Course does not belong to your tenant", status.HTTP_400_BAD_REQUEST
            )

        if student.role != "STUDENT":
            raise AppException(
                "Only students can be enrolled in courses", status.HTTP_400_BAD_REQUEST
            )
...
class LessonProgressCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if user.role != "STUDENT":
            raise AppException(
                "Only students can mark lesson progress", status.HTTP_403_FORBIDDEN
            )

        if lesson.tenant_id != tenant.id:
            raise AppException(
                "Lesson does not belong to your tenant", status.HTTP_400_BAD_REQUEST
            )

        if not Enrollment.objects.filter(
            tenant=tenant, student=user, course=lesson.course
        ).exists():
            raise AppException(
                "You are not enrolled in this course", status.HTTP_403_FORBIDDEN
            )

        if LessonProgress.objects.filter(
            tenant=tenant, student=user, lesson=lesson
        ).exists():
            raise AppException(
                "Lesson progress already exists", status.HTTP_400_BAD_REQUEST
            )
...
class EnrollmentRequestCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if student.role != "STUDENT":
            raise AppException(
                "Only students can request enrollment", status.HTTP_403_FORBIDDEN
            )
        ...
        if Enrollment.objects.filter(
            ...
            raise AppException(
                "You are already enrolled in this course", status.HTTP_400_BAD_REQUEST
            )
        ...
        if EnrollmentRequest.objects.filter(
            ...
            raise AppException(
                "Enrollment request already submitted", status.HTTP_400_BAD_REQUEST
            )
